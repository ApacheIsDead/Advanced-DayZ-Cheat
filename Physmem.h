...
uint64_t SafeTranslateLinearAddress(ULONG_PTR dirbase, ULONG64 virtualAddress)
{
	uint64_t result = 0;
	__try {
		result = TranslateLinearAddress(dirbase, virtualAddress);
	}
	__except (EXCEPTION_EXECUTE_HANDLER) {
		result = 0;  // page walk hit invalid memory, return failure
	}
	return result;
}
...
NTSTATUS WriteProcessMemory(int pid, PVOID Address, PVOID AllocatedBuffer, SIZE_T size, SIZE_T* written)
{
	...
	while (TotalSize)
	{
		// changed to safe version with SEH, as some games have weird page tables that can cause the page walk to hit invalid memory and bugcheck the system. With SEH, we can just return failure for that page and continue writing the rest of the buffer.
		// old version just used TranslateLinearAddress not  "SafeTranslateLinearAdress"
		uint64_t CurPhysAddr = SafeTranslateLinearAddress(process_dirbase, (ULONG64)Address + CurOffset);
		if (!CurPhysAddr)
		{
			KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
				"[-] %s : TranslateLinearAddress failed\n", __FUNCTION__));
...
VOID BeDisableApc(bool bypass)
{
	
	
	PKTHREAD pEthThread = KeGetCurrentThread();

	if (bypass)
	{
		ogFlags.ApcQueueable = pEthThread->ApcQueueable;
		pEthThread->ApcQueueable = 0;
	}
	else
	{
		pEthThread->ApcQueueable = ogFlags.ApcQueueable;
	}
}
